...
project {
    ...
    params {
        ...
        text("github_repository_name", "CobbleForDays", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `CobbleForDays` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "CobbleForDays", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
    ...
    features {
        githubIssues {
            id = "CobbleForDays__IssueTracker"
            displayName = "LexManos/CobbleForDays"
            repositoryURL = "https://github.com/LexManos/CobbleForDays"
        }
    }
}
...
object BuildSecondaryBranches : BuildType({
    ...
    id("CobbleForDays__BuildSecondaryBranches")
...
object PullRequests : BuildType({
    ...
    id("CobbleForDays__PullRequests")
    name = "Pull Requests"
